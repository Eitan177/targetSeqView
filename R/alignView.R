...
function(events,filtsings=TRUE,scoreOnly=FALSE,allowedMM=6,initialExpansion=0,refexpansion=400,
                      indelRate=.005,mmRate=.01,readLength=100,build='hg19',verbose=FALSE){
...
if(verbose) print(paste('Working on event',ii,'of',nrow(events)))
        ...
        indelRate=indelRate,mmRate=mmRate,readLength)
        bamnames=aligned1[[1]]
...
if(verbose) print(paste('primary alignment for event',ii,'done'))
        ...
        returnScoreOnly=scoreOnly,filternames=bamnames,indelRate=indelRate,mmRate=mmRate,readLength)
