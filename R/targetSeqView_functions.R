...
getindelsQuick<-function(readLength,alignFmm,alignRmm,softclippedseq){
        ...
        insertionsF<-do.call(c,mapply(function(x,y){GenomicRanges::setdiff(GenomicRanges::reduce(x),y);GenomicRanges::reduce(x)},insertion(alignFmm),deletion(alignFmm)))

        if(is.null(insertionsF)) insertionsF <- IRanges()
        width(insertionsF)[width(insertionsF)>25]=25

        insertionsR<-do.call(c,mapply(function(x,y){GenomicRanges::setdiff(GenomicRanges::reduce(x),y);GenomicRanges::reduce(x)},insertion(alignRmm),deletion(alignRmm)))
...
getRandomBamRecords <-
    function(bamFile,reverseC=TRUE,recnum=1e5){
        ...
        rngsCheck<-GenomicRanges::unlist(GRangesList(sapply(1:length(chr),function(x){
...
mainAlignQuick <-    function(bamFile,rngsAlign,dedup=TRUE,typeArg ="global-local",substitutionMat=nucleotideSubstitutionMatrix(match = 1, mismatch = -3)[c(1:4,8:9,15),c(1:4,8:9,15)],pairlimit=2e3,gapOpeningArg = -4, gapExtensionArg = -1,indelRate,mmRate,readLength=100,conservativeContigAlign=FALSE,genomeName='Hsapiens',verbose=FALSE){
     ...
     alignedformatted <- doAlignAndformatQuick(bamFile,rngsAlign,dedup,typeArg,substitutionMat,pairlimit,gapOpeningArg ,gapExtensionArg,indelRate=indelRate,mmRate=mmRate,readLength=readLength,conservativeContigAlign=conservativeContigAlign,genomeName=genomeName,verbose=verbose)
